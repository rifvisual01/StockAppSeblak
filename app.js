...
function getDefaultViewForCurrentUser() {
  return window.currentUser?.role === 'admin' ? 'dashboard' : 'transaction';
}

function isValidViewForCurrentUser(view) {
  const staffViews = ['transaction', 'history'];
  const adminViews = ['dashboard', 'items', 'reports', 'users'];
  const knownViews = [...staffViews, ...adminViews];
    ...
    <div class="login-card w-full max-w-md fade-in relative z-10">
      ...
      <div class="text-center mb-6 sm:mb-8">
        ...
        <div class="inline-flex items-center justify-center mb-3 mt-2 relative">
          ...
          <img src="./seblak-nyakcut-logo.png" alt="Seblak Nyakcut" class="login-app-icon w-24 h-24 sm:w-32 sm:h-32 rounded-3xl shadow-2xl transform hover:scale-105 transition-transform duration-300 object-cover">
        </div>
    ...
    <header class="app-header bg-gradient-to-r from-red-600 via-red-500 to-red-600 px-4 lg:px-6 py-4 flex items-center justify-between shrink-0 shadow-lg z-50">
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="min-w-0">
            <div class="font-800 text-white text-base leading-tight truncate">${window.currentUser.name}</div>
            <div class="text-white/90 text-xs font-800 mt-0.5 truncate">${window.currentUser.role==='admin'?'Admin':'Staff'}</div>
          </div>
        </div>
      </div>
      <div class="flex items-center gap-3">
