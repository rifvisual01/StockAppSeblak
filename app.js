            ...
            ${catItems.map((item, idx) => {
              ...
              <div class="w-full p-4 ${isOutOfStock ? 'bg-gray-50 opacity-75' : 'hover:bg-red-50'} transition flex items-center justify-between group">
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-xs text-gray-500 mt-1">${formatCurrency(item.price)} · ${itemUsesStock(item) ? `Stok: ${item.stock}` : 'Tanpa stok'}${isOutOfStock ? ' · Habis' : ''}</div>
                </div>
    ...
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-40 p-4 fade-in">
      <div class="modal-box bg-white rounded-2xl shadow-2xl w-full max-w-4xl p-6 max-h-96 overflow-auto">
        ...
        <div class="flex gap-2 pt-3 border-t border-gray-200">
          ...
          <button id="btn-submit-tx-batch" class="flex-1 py-2.5 rounded-xl ${txType==='IN'?'bg-emerald-600 hover:bg-emerald-700':'bg-red-600 hover:bg-red-700'} text-white text-sm font-600 btn-primary">Simpan ${txType==='IN'?'Stok Masuk':'Stok Keluar'}</button>
        </div>
      </div>
    </div>
    ` : ''}
